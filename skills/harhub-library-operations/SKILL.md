...
description: Inspect and manage Harhub workspace Library Skills and MCP configurations through the Harhub MCP server. Use when an agent needs to list, inspect, validate, upload, version, download, share, unshare, delete, or install Harhub assets.
---

# Harhub Library Operations

Use Harhub as the source of truth for shared Agent Skills and private MCP
configurations. Read the current asset before changing it, and report asset IDs
and versions after mutations.

## Workflow

1. Call `harhub_whoami` when the active workspace is not already established.
2. Discover candidates with `harhub_assets_list`; pass `kind: "skill"` or
   `kind: "mcp"` to scope the result.
...
- Upload one MCP JSON: `harhub_mcp_upload`. Use environment placeholders for
  credentials; never put access tokens in the description.
...
Sharing, unsharing, and public installation are Skill-only. Unsharing,
installing, and deleting require `confirm: true`. Ask for confirmation if the
user has not explicitly requested the action.
