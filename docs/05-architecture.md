...
- `harhub_state` 保存 accounts、sessions、workspaces、memberships、invitations、device authorization 和 asset shares。
- `harhub_workspace_catalogs` 保存每个 workspace 的 Skill catalog 和 Asset catalog。
- Imported Skill files 按 Skill 分隔存储在 S3/S3-compatible object storage，不进入数据库，也不保留源 zip。
...
当前 share 是 workspace member 为 uploaded Skill 创建的可撤销 public bearer link。
...
- `/api/workspaces/{workspaceId}/assets`：列表、详情、文件 preview、multi-Skill upload、validate、bulk validate/delete 和 delete。
...
- Imported Skill files 以独立 prefix 存储在 S3-compatible object store；标准 zip 在下载时动态生成。
